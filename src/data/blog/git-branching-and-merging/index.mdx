...
When you run `$ git init` or `$ git clone` commands to create a new repository, Git automatically creates a _master_ branch for you, and points development to it.
...
That's easy, you just run `$ git branch`.
If you run that right after `$ git init`, you should see something like:

```bash
* master
```

That means you only have _master_ branch, and it's the current one, indicated by the `*` sign.
...
To create a new branch, you run a command like `$ git branch [branch-name]`, for example:
...
If you run `$ git branch` now, you should see something like:
...
You need to run a command like `$ git checkout [branch-name]` to switch between branches, for example:
...
If we run `$git branch` now, we should see something like:
...
Because you didn't make any changes in your _master_ branch **after** creating your new _testing_ branch, running `$ git merge` command just moves _master_ forward to point to the last commit in the _testing_ branch, which in this case is the next commit, but you could have made _n_ commits in _testing_ before merging it.
To merge two branches, you have to point to the branch you want to merge into (_master_ in this case, which will receive changes from _testing_) and run a `$ git merge` command:
...
At this point you can delete your _testing_ branch by running a command like `$ git branch -d [branch-name]`, for example:
...
Let's go back to our _master_ branch by running `$ git checkout master` (remember: when you switch between branches, your working directory changes too).
...
So, in such case, what will Git do after we run `$ git merge`?
Go ahead and give it a try!

Git will do a simple three-way merge, using the last snapshot (commit) of each branch plus the common ancestor of both (`f30ab` in Figure 3).
...
If you run `$ git status` now, Git will show you an _Unmerged paths_ section listing files that are in a conflict state (_both modified_).
...
Everything that is below `<< HEAD` and above `==` is the _HEAD_ version of the file (the last snapshot of the current branch when you run `$ git merge`), in this case, the _master_ branch version of the file.
And everything above `>> testing` and below `==` is the version of the file in the other branch (_testing_, in this case).

From the Git perspective, to fix that conflict you just have to remove those Git markers, manually, and save the file.
Also, if you prefer you can run `$ git mergetool` to open a visual merge tool.

After removing those markers, you need to tell Git that you've fixed that conflict.
You do that by running `$ git add`:
