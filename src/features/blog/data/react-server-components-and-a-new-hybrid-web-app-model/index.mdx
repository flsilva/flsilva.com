...
export const date = "2023-12-18T00:00:00";
...
export const heroImageCreditUrl =
  "https://www.freepik.com/free-vector/atomic-science-technology-background-vector-border-blue-neon-style-with-blank-space_17213335.htm";
...
export const slug = "react-server-components-and-a-new-hybrid-web-app-model";

export const tags = ["react", "nextjs"];

export const title = "React Server Components and a new hybrid web app model";
...
- [A brief overview of web application models](#a-brief-overview-of-web-application-models)
  - [1990s](#1990s)
  - [2000s](#2000s)
  - [2010s](#2010s)
  - [2016: Next.js FTW](#2016-nextjs-ftw)
  - [Next.js Pages Router's issues](#nextjs-pages-routers-issues)
    - [Pages Router's issue #1](#pages-routers-issue-1)
    - [Pages Router's issue #2](#pages-routers-issue-2)
    - [Pages Router's issue #3](#pages-routers-issue-3)
    - [Pages Router's issue #4](#pages-routers-issue-4)
- [A new hybrid web application model](#a-new-hybrid-web-application-model)
  - [2023: Next.js App Router FTW](#2023-nextjs-app-router-ftw)
- [What are React Server Components?](#what-are-react-server-components-rscs)
- [What are Client Components?](#what-are-client-components)
- [React Server Components vs SSR](#react-server-components-vs-ssr)
- [React Server Components constraints](#react-server-components-constraints)
  - [Hooks](#hooks)
  - [Interactivity](#interactivity)
  - [Browser APIs](#browser-apis)
  - [Continuous updates](#continuous-updates)
  - [Data serialization](#data-serialization)
  - [Too many constraints?](#too-many-constraints)
- [React Server Components benefits](#react-server-components-benefits)
  - [Security](#security)
  - [Zero-Bundle-Size impact](#zero-bundle-size-impact)
  - [Automatic client code splitting](#automatic-client-code-splitting)
  - [SSG](#ssg)
  - [Server-side data fetching](#server-side-data-fetching)
  - [The key to unlocking a new hybrid web application model](#the-key-to-unlocking-a-new-hybrid-web-application-model)
- [Server Components must be pure functions](#server-components-must-be-pure-functions)
- [How to write Server and Client Components](#how-to-write-server-and-client-components)
- [What are Shared Components?](#what-are-shared-components)
...
- [Building full-stack React components](#building-full-stack-react-components)
  - [Different data sources](#different-data-sources)
- [Server Actions](#server-actions)
- [How to re-render Server Components](#how-to-re-render-server-components)
  - [The Request/Response Lifecycle](#the-requestresponse-lifecycle)<br />
  - [Using revalidatePath() and revalidateTag()](#using-revalidatepath-and-revalidatetag)<br />
  - [Using router.refresh()](#using-routerrefresh)<br />
  - [React preserves the UI state](#react-preserves-the-ui-state)<br />
...
SPAs became the norm. We took a client-centric model of rendering everything on the client ([CSR](https://web.dev/articles/rendering-on-the-web#client-side_rendering)). In this model, we render nothing on the server, returning an empty HTML instead, having the client load and execute a large amount of JavaScript code to make several HTTP requests to the server to fetch data, and finally, rendering an interactive HTML application that only updates the necessary parts of the page! 😅

And this time, we're not eating spaghetti! Long-live SPAs! 🎉
...
We have to use proprietary _`getServerSideProps()`_ and _`getStaticProps()`_ functions at a top-level page component to fetch data server-side and drill it down to the necessary components. **We cannot fetch data server-side at the component level.**
...
<InfoBoxQuotation>
  We're moving parts of the code back to the server while removing others,
  making the client and the server **lightweight again**.
</InfoBoxQuotation>
...
A React Server Component is a new kind of React component designed to run only on the server ([or at build time](#ssg)) and render static HTML<sup>[1]</sup> that's never hydrated on the client, extending the React programming model.
...
React can only update Server Components on the server where their JavaScript code runs. And it does that by using the traditional [request/response lifecycle](https://nextjs.org/docs/app/building-your-application/rendering#request-response-lifecycle) of the web ([more on this later](#how-to-re-render-server-components)).
...
However, we can pass Server Actions to interactive events like _`onClick()`_—more on this [next](#data-serialization).
...
We cannot pass functions, like event handlers, from Server Components to Client Components, but we can pass [Server Actions](#server-actions).
...
<InfoBoxQuotation>
  By embracing Server Components, your bundle size **doesn't grow linearly** as
  your app grows with new features as it does with Client Components.
</InfoBoxQuotation>

You can consume heavyweight dependencies on the server without affecting the user experience.
...
That means we don't need a server to use Server Components, and we can use them to generate static HTML pages and complete static websites that can be served from a CDN, making them the ultimate React [SSG](https://www.patterns.dev/react/static-rendering) tool!
...
<InfoBoxQuotation>
  We originally thought of Server Components as a way to solve the waterfall
  problem. —Dan Abramov, [on Dec 21st,
  2020](https://www.youtube.com/watch?v=TQQPAU21ZUw&t=687s&ab_channel=MetaOpenSource).
</InfoBoxQuotation>
...
<InfoBoxQuotation>
  We should use Server Components as much as possible due to their numerous
  benefits, including zero impact on bundle size and server-side data fetching,
  [moving Client Components down the
  tree](https://nextjs.org/docs/app/building-your-application/rendering/composition-patterns#moving-client-components-down-the-tree).
</InfoBoxQuotation>
...
<InfoBoxQuotation>
  We can now build highly reusable and composable full-stack components that
  fetch data and render the UI in the server in a single roundtrip with Server
  Components.
</InfoBoxQuotation>
...
Well, it turns out that before the App Router, we had to use _`getServerSideProps()`_, which must come at the top-level `page` component. That means we cannot have _`<TaskList>`_ fetch its data if we want to fetch data on the server. 🙄

That also means the _`<TaskList>`_'s composability is hurt because we cannot just throw it in a component tree. We must _`getServerSideProps()`_ and drill it down whenever we use it! 😣

Enter Server Components and App Router. 💡

<InfoBoxQuotation>
  With Server Components, we can effortlessly fetch data on the server at the
  component level for the first time ever! 🤯
</InfoBoxQuotation>
...
<InfoBoxQuotation>
  I call this component a full-stack component: a self-contained data-driven
  Server Component that's fully reusable and composable despite its data
  dependencies.
</InfoBoxQuotation>
...
Note that _`<TaskList>`_ cannot be decoupled from task _data_. Its _purpose_ is to render such data.
...
However, we're still fetching data for all Server Components before rendering the application; the Pages Router's [issue #2](#pages-routers-issue-2). Let's see how to solve that.
...
<InfoBoxQuotation>
  React is now a full-stack component-based solution, and it's a rewarding time
  to be a full-stack engineer!
</InfoBoxQuotation>

Many use cases exist where this new hybrid web application model doesn't fit. It's not a silver bullet in any way. But it's an incredible tool for those use cases where it does fit. It's a huge leap forward and the future of web development for different types of web applications. We're fixing non-trivial issues and improving the quality of web applications while reducing complexity, code, and costs. It's definitely a win-win.

However, it's also a mental shift that will take time for the community to embrace.

But we will embrace it and build reusable and composable Server Components. We already have. CodeHike's [Bright](https://bright.codehike.org/) is a great example.
...
I'm excited to build apps with React Server Components and the new Next.js App Router. 🚀
...
<InfoBox>
Does this new hybrid web application model make sense for you? Share your insights in the [comments below](#post-comments). I'd love to hear your thoughts.
...
<SocialMediaShareButtons
  ...
  tweetVia="flsilva7"
  url="https://www.flsilva.com/blog/react-server-components-and-a-new-hybrid-web-app-model"
/>
...
- [Everything I wish I knew before moving 50,000 lines of code to React Server Components (Darius Cepulis)](https://www.mux.com/blog/what-are-react-server-components)
...
- [Making Sense of React Server Components (Joshua Comeau)](https://www.joshwcomeau.com/react/server-components/)
- [React Server Components – How and Why You Should Use Them in Your Code (Tapas Adhikary)](https://www.freecodecamp.org/news/how-to-use-react-server-components/)
...
- [React v18.0 (March 2022)](https://react.dev/blog/2022/03/29/react-v18)
- [React 18 Keynote (React Conf 2021)](https://www.youtube.com/watch?v=FZ0cG47msEk&list=PLNG_1j3cPCaZZ7etkzWA7JfdmKWT0pMsa&ab_channel=ReactConf2021)
